{if strpos($smarty.server.HTTP_ACCEPT, "application/json") === false}
  ...
  <script type="text/javascript">
    window.piwigoData = {
      ...
      aptThemeStatus: {
        {if not empty($apt_theme_satus_json)}
          {foreach from=$apt_theme_satus_json item=val}
            "{$val[0]}": {$val[1]},
          {/foreach}
        {/if}

        "isPluginInstalled": {isset($apt_plugin_available) && $apt_plugin_available ? "true" : "false"},
      }
    };
  </script>
